...
export const PLATFORM_SETUP: PlatformSetup[] = [
  {
    ...
    steps: [
      ...
      'Under Bot Permissions, tick: View Channels, Read Message History, and View Audit Log (needed for ban stats).',
      ...
      'Back in Catch → Integrations → Discord → Connect. Paste the Bot Token and the Server ID, then click Connect.',
    ],
  ...
  },
  {
    ...
    steps: [
      ...
      'Make the bot an Admin of the group (required to read the member count).',
      'Get the Chat ID: temporarily add "@RawDataBot" to the group — it posts the group id (a negative number like -1001234567890). Copy it, then remove that bot.',
      'Back in Catch → Integrations → Telegram → Connect. Paste the Bot Token and the Chat ID, then click Connect.',
    ],
...
export const INSTRUCTION_SECTIONS: InstructionSection[] = [
  ...
  },
  {
    ...
    metrics: [
      ...
      },
      {
        ...
        how: 'Summed from Discord channel messages and Telegram group messages.',
  ...
  },
  {
    ...
    metrics: [
      ...
      },
      {
        ...
        what: 'Time between an incident alert and the moderator’s action.',
        how: 'Measured from alert timestamp to logged action timestamp.',
        why: 'Target under 5 minutes — slow responses let scams spread further.',
      },
      {
        name: 'Bans Executed',
        what: 'Total bans and kicks performed by the moderator.',
        how: 'Auto-synced from Discord when the integration is active.',
  ...
  },
  {
    ...
    steps: [
      ...
      'Once connected, use Sync Now any time you want a fresh pull outside the automatic hourly sync.',
    ],
    ...
    explainedItems: [
      {
        name: 'Discord',
        description:
          'Create a bot at discord.com/developers, copy the Bot Token and Server ID, and paste them into Catch. The bot must be added to the server with read permissions.',
      },
      {
        name: 'Telegram',
        description: 'Create a bot via @BotFather, get the Bot Token, add the bot to the group as admin, then paste the Chat ID into Catch.',
      },
      {
