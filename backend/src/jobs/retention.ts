...
import { platformMetricSnapshots, processedTelegramUpdates, securityEvents } from '../db/schema/index.js'
...
export async function runRetention(now = new Date()) {
  ...
  return { snapshots: snapshots.length, securityEvents: events.length, telegramDedup: dedup.length }
}
