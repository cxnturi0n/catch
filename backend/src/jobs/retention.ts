...
import { platformMetricSnapshots, processedTelegramUpdates, securityEvents, aiConversations, platformMessages } from '../db/schema/index.js'
...
// Message text lives 30 days (AI analysis window); the counters derived from
// it (member_messages, message_activity, channel_activity) are kept.
export const MESSAGES_RETENTION_DAYS = 30
...
export async function runRetention(now = new Date()) {
  ...
  const messages = await db.delete(platformMessages).where(lt(platformMessages.sentAt, cutoff(MESSAGES_RETENTION_DAYS))).returning({ id: platformMessages.id })
  ...
  return { snapshots: snapshots.length, securityEvents: events.length, telegramDedup: dedup.length, chats: chats.length, messages: messages.length }
}
