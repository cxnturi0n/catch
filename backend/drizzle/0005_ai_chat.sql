...
CREATE INDEX "ai_conversations_ws_user_idx" ON "ai_conversations" USING btree ("workspace_id","user_id","updated_at");--> statement-breakpoint
CREATE INDEX "ai_messages_conv_idx" ON "ai_messages" USING btree ("conversation_id","created_at");--> statement-breakpoint
ALTER TABLE "help_docs" ADD COLUMN "tsv" tsvector GENERATED ALWAYS AS (to_tsvector('english', coalesce("title", '') || ' ' || coalesce("body", ''))) STORED;--> statement-breakpoint
CREATE INDEX "help_docs_tsv_idx" ON "help_docs" USING gin ("tsv");
